...
def get_files():
    ...
    with SessionLocal() as db:
        ...
        requests_ids = list(map(lambda r: r.id, requests))
        allRequestResponses = db.query(Results).filter(
            Results.id.in_(requests_ids)).all()
        for req in requests:
            filename = req.fileName
            file_path = os.path.join(file_directory, filename)
            res = next(
                filter(lambda item: item.requestId == req.id, allRequestResponses),
                None,
            )
            if not res:
                continue
